...
class FileRoutingRule(BaseRule):
    ...
    def evaluate(self, context: PRContext, client: GitHubAPIClient) -> RuleResult:
        ...
        if not matched_any and Label.UNDER_REVIEW not in context.labels:
            labels_to_add.add(Label.NEEDS_TRIAGE)
    ...
    def evaluate(self, context: PRContext, client: GitHubAPIClient) -> RuleResult:
        ...
        for review in context.reviews:
            if review.user in SUPERPOWER_USERS and review.state == "APPROVED":
                ...
                labels_to_add.add(Label.APPROVED)
                labels_to_add.add(Label.READY_TO_MERGE)
                return RuleResult(
                    self.name,
                    satisfied=True,
                    labels_to_add=labels_to_add,
        ...
        for rule in self.config:
            ...
            for team_handle, req_details in review_reqs.items():
                ...
                if needs_label == Label.NEEDS_TC_REVIEW and Label.TC_MAJORITY_APPROVED in context.labels:
        ...
        if Label.TC_MAJORITY_APPROVED in context.labels:
            ...
            if event_user and context.event_name == "pull_request" and context.event_payload.get("action") == "labeled":
                ...
                if label_added == Label.TC_MAJORITY_APPROVED:
                    is_tc = client.check_team_membership("Universal-Commerce-Protocol", "tech-council", event_user)
                    is_devops = client.check_team_membership("Universal-Commerce-Protocol", "devops-maintainers", event_user)
                    if not is_tc and not is_devops:
                        print(f"[GUARDRAIL] Unauthorized user {event_user} applied majority label. Revoking.")
                        labels_to_remove.add(Label.TC_MAJORITY_APPROVED)
                        comments.append(
                            f"Warning: @{event_user}, you do not have permission to apply "
                            f"`{Label.TC_MAJORITY_APPROVED}`. This action has been automatically reverted."
                        )
        ...
        if all_rules_satisfied and rules_evaluated > 0:
            ...
            labels_to_remove.add(Label.APPROVED)
            labels_to_remove.add(Label.READY_TO_MERGE)
...
class LabelLifecycleRule(BaseRule):
    ...
    def evaluate(self, context: PRContext, client: GitHubAPIClient) -> RuleResult:
        ...
        if context.event_name == "pull_request" and event_action == "labeled":
            label_added = context.event_payload.get("label", {}).get("name")
            if label_added == Label.BLOCKED:
                labels_to_remove.add(Label.UNDER_REVIEW)
        ...
        if context.event_name == "pull_request" and event_action == "synchronize":
            if Label.BLOCKED in context.labels:
                labels_to_add.add(Label.UNDER_REVIEW)
                labels_to_remove.add(Label.BLOCKED)
        ...
        if context.event_name == "issue_comment" and event_action == "created":
            ...
            if comment_author == context.author and Label.BLOCKED in context.labels:
                labels_to_add.add(Label.UNDER_REVIEW)
                labels_to_remove.add(Label.BLOCKED)
...
class StalePRRule(BaseRule):
    ...
    def evaluate(self, context: PRContext, client: GitHubAPIClient) -> RuleResult:
        ...
        if is_inactive_abandon in context.labels:
            if Label.ABANDON_CANDIDATE not in context.labels:
                labels_to_add.add(Label.ABANDON_CANDIDATE)
                labels_to_add.add(Label.NEEDS_TRIAGE)
                labels_to_add.add(Label.STALE_REVIEW)
                comments.append(
                    f"This pull request has been blocked and inactive for "
                    f"{self.abandon_days} days. "
                    f"It has been marked as an `{Label.ABANDON_CANDIDATE}`. "
                    f"Please resolve the blockers to resume review."
                )

        # Scenario B: PR waiting on reviews
        elif is_inactive_stale and Label.UNDER_REVIEW in context.labels:
            if Label.STALE_REVIEW not in context.labels:
                labels_to_add.add(Label.STALE_REVIEW)
                labels_to_add.add(Label.NEEDS_TRIAGE)
                comments.append(
...
def verify_team_approvals(context: PRContext, team_handle: str, threshold: any, client: GitHubAPIClient) -> Tuple[bool, int]:
    ...
    if threshold == "majority":
        ...
        return Label.TC_MAJORITY_APPROVED in context.labels, approvals
